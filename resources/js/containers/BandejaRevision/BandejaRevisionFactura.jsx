...
const BandejaRevisionFactura = () => {
  ...
  const [estatusActividad, setEstatusActividad] = useState(8);

  const onCancelarAccion = async (id) => {
    if (window.confirm("Seguro que desea regresar la factura")) {
      const estatus = 7;
      const res = await bandejaActividades.revisionAsignacion(id, estatus);
      toast("Regresar la factura", { type: "error", autoClose: 2000 });
      await getAsignaciones(estatusActividad);
    }
  };

  const onSolicitarAccion = async (id) => {
    if (window.confirm("Seguro que desea autorizar la documentacion")) {
      const estatus = 9;
      const res = await bandejaActividades.revisionAsignacion(id, estatus);
      toast("Documentacion Autorizada", { type: "success", autoClose: 2000 });
      await getAsignaciones(estatusActividad);
    }
  };
  ...
  return (
    <div className="BandejaAct">
      <Rutas ruta={"Bandeja de Revision"} />
      ...
      <div className="d-flex ">
        ...
        >Revision Documentos</Link>
        <button
          className="btn btn-info ml-1 p-2"
          disabled={true}
        >Revision Factura</button>
      </div>
...
export default BandejaRevisionFactura;
